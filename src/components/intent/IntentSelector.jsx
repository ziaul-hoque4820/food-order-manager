...
export default function IntentSelector() {
    ...
    return (
        <div className="h-full bg-slate-800 flex flex-col overflow-hidden">
            <div className="h-full max-w-7xl mx-auto w-full flex flex-col p-4 md:p-8">

                {/* Header - Fixed */}
                <div className="text-center mb-6 flex-shrink-0">
                    <p className="text-gray-100 text-xl font-bold md:text-2xl">
                ...
                {/* Cards Container - Flexible with hidden scrollbar */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-fit">
                        {intentOptions.map((option) => (
                            <div
                                ...
                                className={`group relative h-64 lg:h-72 rounded-xl overflow-hidden cursor-pointer transition-all duration-500 transform ${selectedIntent === option.id ? "ring-4 ring-red-400 scale-[1.03]" : "hover:scale-[1.02]"}
                            `}
                            >
                                {/* Background Image */}
                                <div
                                ...
                                <div className="relative z-10 h-full flex flex-col justify-end p-6 text-white">
                                    <div className="flex items-center justify-between group-hover:text-cyan-400 transition-colors duration-300">
                                        <div>
                                            <div className="text-3xl lg:text-4xl mb-2">{option.emoji}</div>
                                            <h3 className="text-xl lg:text-2xl font-bold">{option.title}</h3>
                                            <p className="text-xs lg:text-sm text-gray-200">{option.subtitle}</p>
                                        </div>
